...
class message_pops_up(models.TransientModel):
    ...
    def button_confirm(self):
        ...
        if records:
            getattr(records, call_back_func)()
        else:
            if hasattr(self.active_model, call_back_func):
                getattr(self.active_model, call_back_func)()
            else:
                _logger.warning(f"[MOMMY]we couldn't find any proper model which has call function: {call_back_func}")